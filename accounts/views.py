        ...
        if user is not None:
            try:
                ...
                if is_cart_item_exists:
                    ...
                    product_variation=[]# ye wo variation jayege jo without login kiye add kare hai
...
def forgotPassword(request):
    if request.method=="POST":
        ...
        if Account.objects.filter(email=email).exists():
            ...
            send_mail=EmailMessage(mail_subject,message,to=[to_email])
            send_mail.send()
            messages.success(request,"Password reset email have been sent on ur email address")
            return redirect('login')
        else:
